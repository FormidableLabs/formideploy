...
const isRollback = (archive) => {
  if (archive.endsWith(".json")) { return true; }
  if (archive.endsWith(".tar.gz")) { return false; }
  ...
  return data.Contents.map((obj) => {
    ...
    return {
      ...
      meta: {
        ...
        deployType: isRollback(name) ? "rollback" : "deploy",
        gitSha,
        gitState,
        size: obj.Size
      }
  ...
  const results = table(
    [
      ["Deploy Date", "Type", "Git SHA", "Git State", "Name",
        "Size"].map((s) => chalk.gray.bold(s)),
      ...objs.map(({ name, meta: { deployDate, deployType, gitSha, gitState, size } }) =>
        [
...
const downloadArchive = async ({ archive }) => {
  ...
  // Short-circuit if rollback.
  if (isRollback(archive)) {
    return {
      rollback: zipPath
    };
  }
...
const resolveArchive = async ({ archive }) => {
  // Short circuit if already an archive.
  if (!isRollback(archive)) { return archive; }

  // If rollback, get data and convert.
  const { rollback } = await downloadArchive({ archive });
  const { original } = JSON.parse(await fs.readFile(rollback));

  // Normalize name from /PATH/TO/archive.tar.gz -> archive.tar.gz
  const resolved = path.basename(original.name);
  log(chalk `Resolved {red rollback} {cyan ${path.basename(archive)}} to `
    + chalk `{green deploy} {cyan ${resolved}} to `);
