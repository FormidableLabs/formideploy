...
const getArchive = async ({ archive }) => {
...
const displayArchive = async ({ archive }) => {
  const { name, meta } = await getArchive({ archive });
  ...
  const results = Object.entries(meta)
    ...
    .join("\n");
  log(chalk `Metadata for archive: {cyan ${name}}\n${results}`);
};
...
const archives = async ({ limit, start, archive }) => {
  ...
  if (archive) {
    ...
    await displayArchives({ limit, start });
  }
};
