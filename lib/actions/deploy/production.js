...
const awsPublish = async ({ dryrun, url }) => {
  ...
  // Create redirects.
  //
  // - [ ] TODO HERE: Have **removed** REDIRECTS from fmd.com build.
  //      -> e.g. `(dryrun) delete: s3://tmp-experiment-02.formidable.com/careers/index.html`
  // - [ ] TODO: Maybe switch to just putObject API instead?
  // - [ ] TODO: Remove REDIRECTS from formidable build and rebuild site?
  //      - Or maybe --exclude so s3 sync doesn't nuke them always...?
  //
  ...
  await Promise.all(Object.entries(redirects)
    .map(([src, dest]) => dryrun
      ? write(`(dryrun) redirect: ${s3Path(redirectFile(src))} to ${dest}\n`)
      : exec({
        cmd,
        args: [
          "s3api",
