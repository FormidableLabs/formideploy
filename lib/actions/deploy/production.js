...
const { downloadArchive, getArchiveMeta, displayArchiveMeta } = require("../archives");
...
const createRollback = async ({ archive }) => {
  ...
  const meta = Object.entries({
    ...
    "build-job-id": build.jobId,
    "build-job-url": build.jobUrl,
    "original-name": archiveMeta.name,
...
const production = async ({ dryrun, archive }) => {
  ...
  if (await isAuthenticated()) {
    // Create archive or rollback.
    archiveInfo = await (archive ? createRollback({ archive }) : createArchive());
