...
const { write, getLoggers, getFiles, exec } = require("../../util");
...
const awsPublish = async ({ dryrun, url }) => {
  ...
  // - [ ] TODO: formidops needs IAM permissions for ^^^
  // - [ ] TODO: Can we query the distribution ID or need to hard-code it?
  //
  ...
  const dist = await s3cmd({
    ...
    .then((results) => {
      ...
      if (results.NextToken) {
        // We take a shortcut and assume we don't have enough distributions
        // to trigger pagination. Throw in an assert here to not miss anything.
        // If hitting this error, please reprogram to traverse the paginated
        // results.
        throw new Error("Received NextToken paginated cloudfront distribution results");
      }

      // Match on aliased domains to find our actual distribution id.
      return results.DistributionList.Items
        .filter(({ Aliases: { Items } }) => Items.includes(domain));
    })
    .then((dists) => {
