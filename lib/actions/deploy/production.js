...
const awsPublish = async ({ dryrun, url, archive }) => {
  ...
  // TODO(20): Different message / logic on rollback.
  log(chalk `Uploading archive to: {cyan s3://${archive.dest.bucket}/${archive.dest.path}} `
    + chalk `with metadata: {gray ${JSON.stringify(archive.meta)}}`);

  // // TODO(formideploy): Figure out archive upload strategy.
  // // Upload archive
  // logMsg(
  //   chalk`\n* (Start) Uploading new site archive with metadata: {gray ${JSON.stringify(
  //     metadata
  //   )}}`
  // );
  // await spawn(
  ...
  //     archive,
  //     `s3://${BUCKET_ARCHIVES}/${archive}`
  //   ].filter(Boolean)
  // );
  // logMsg("\n* (End) Successfully uploaded archive.");
};

// Produce all applicable archive information.
//
// TODO(20): Get metadata from _original_ archive and integrate into new meta?
// https://github.com/FormidableLabs/formideploy/issues/20
// - "rollback-original-archive": "/PATH/TO/ORIGINAL/OBJECT"
// ... OR ...
// - Just put a placeholder empty / minimal data file there? (With meta)
...
const destroyArchive = async ({ archive }) => {
  log(chalk `Removing local archive {gray ${archive.zip.path}} from {gray ${archive.src.path}}`);
  await exec({
    cmd: "rm",
    args: [archive.zip.path],
    log
  });
};
