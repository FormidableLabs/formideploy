...
const awsPublish = async ({ dryrun, url }) => {
  ...
  const dist = await s3cmd({
    ...
    .then((results) => !results ? null : results.DistributionList.Items
  ...
  console.log("TODO HERE", { dist });

  log(chalk `Invalidating CDN {cyan ${dist.Id}} ({cyan ${dist.DomainName}}) `
    + chalk `cache path {cyan TODO_PATH}`);
};
