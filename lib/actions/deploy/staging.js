...
const staging = async ({ dryrun }) => {
  ...
  const bucket = production.bucket;
  ...
  const archivePath = `${production.domain}/${site.basePath}/${archiveName}`;
  const archiveMeta = {
  ...
  console.log("TODO HERE TRAVIS VARS", Object.entries(process.env)
    .filter(([k]) => k.startsWith("TRAVIS_"))
