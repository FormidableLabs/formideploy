...
const { config: { build, site } } = require("../config");
...
const serve = async ({ port }) => {
  ...
  const buildPath = path.join(build.dir, site.basePath);
  ...
  addHandlers(server);
  server.listen(port, () => {
    log(chalk `Serving build from "{cyan ${buildPath}}" at: {cyan.underline ${address}}`);
  });
};
