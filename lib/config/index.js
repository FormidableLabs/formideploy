...
const getJobId = () => {
  const { TRAVIS_JOB_ID, CIRCLE_BUILD_NUM } = process.env;
...
const getJobUrl = () => {
  const { TRAVIS_JOB_WEB_URL, CIRCLE_BUILD_URL } = process.env;
...
const getConfig = () => {
  ...
  // General: first pass.
  cfg.build.jobdId = cfg.build.jobId || getJobId();
  cfg.build.jobdUrl = cfg.build.jobUrl || getJobUrl();
