...
// Create a proxy handler that tracks nested property paths and validates that
// fields are not undefined or null.
class RequiredHandler {
  constructor({ parts = [], maxLevel = -1 } = {}) {
    this.parts = parts;
    this.maxLevel = maxLevel;
  }

  get(obj, prop) {
    // Hack to avoid needless proxies by having an `__isProxy` key
    if (prop === "__isProxy") { return true; }

    const parts = this.parts.concat(prop); // Accumulate path parts.
    const val = obj[prop];

    // Validation.
    if (typeof val === "undefined" || val === null) {
      throw new Error(chalk `Missing config value for {gray ${parts.join(".")}}`);
    }

    // Add proxies of objects up to "max level".
    const { maxLevel } = this;
    if (maxLevel > 0 && maxLevel >= parts.length
      && !val.__isProxy && typeof val === "object" && val !== null) {
      return new Proxy(val, new RequiredHandler({ parts, maxLevel }));
    }
...
const getBuildId = () => {
  // TODO(staging): Travis detect merge build vs. branch build vs. master merge ???
  // TODO(staging): Add log message about "not PR, skipping".
...
const getConfig = () => {
  ...
  // General: first pass.
  raw.build.id = raw.build.id || getBuildId();

  // Lander vs. base site specifics.
  if (raw.lander.name) {
    // Lander mutations
    raw.site.basePath = raw.site.basePath || `open-source/${raw.lander.name}`;
    raw.domain.staging = raw.domain.staging
      || `formidable-com-${raw.lander.name}-staging-${raw.build.id}.surge.sh`;
  } else {
    // Base website mutations
    raw.site.basePath = raw.site.basePath || "";
    raw.domain.staging = raw.domain.staging || `formidable-com-staging-${raw.build.id}.surge.sh`;
  }

  // General: second pass.
  raw.build.path = raw.build.path || path.join(raw.build.dir, raw.site.basePath);

  return {
    // The raw configuration object
    raw,
    // Same object but all path accesses validate path is not null/undefined
    required: new Proxy(raw, new RequiredHandler({ maxLevel: 2 }))
  };
};

// Validate needed parts of all real actions.
const validate = async ({ required }) => {
  // General
  if (!await exists(required.build.path)) {
    throw new Error(chalk `Build output missing at: {cyan ${required.build.path}}`);
  }
};
