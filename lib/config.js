...
const validate = async (cfg, { isDeploy }) => {
  // General
  if (typeof cfg.site.basePath !== "string") {
    throw new Error(chalk `Missing config value for {gray site.basePath}`);
  }
  ...
  if (isDeploy) {
    if (!cfg.domain.staging) {
      throw new Error(chalk `Missing config value for {gray domain.staging}`);
    } else if (!cfg.domain.production) {
      throw new Error(chalk `Missing config value for {gray domain.production}`);
    }
  }
};
