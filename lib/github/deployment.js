...
class Deployment {
  constructor({ environment, url, dryrun = false, log, error }) {
    // GITHUB_TOKEN is present by default in GitHub actions.
    const { GITHUB_DEPLOYMENT_TOKEN, GITHUB_TOKEN } = process.env;

    this.auth = GITHUB_DEPLOYMENT_TOKEN || GITHUB_TOKEN;
    ...
    this.enabled = !this.dryrun && this.auth && (
      ...
      this.isProduction
      // In a real PR
      || build.changeId && build.isPullRequest
    );
  ...
  get disabledReason() {
    ...
    if (!this.isProduction && (!build.changeId || !build.isPullRequest)) {
      return "not a pull request for staging";
    }
  ...
  async start() {
    ...
    await client.repos.createDeployment({
      ...
      ref: sha,
      environment,
      production_environment: this.isProduction,
      description:
